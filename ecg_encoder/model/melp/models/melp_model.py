...
from ecg_encoder.model.melp.models.merl_model import MERLModel
from ecg_encoder.model.melp.backbone.transformer import  (
    LayerNorm,
    QuickGELU,
    MultimodalTransformer,
)
from ecg_encoder.model.melp.utils.openclip_loss import CoCaLoss
from ecg_encoder.model.melp.models.ecgfm_model import ECGFMModel
from ecg_encoder.model.melp.backbone.transformer import AttentionalPooler
from ecg_encoder.model.melp.backbone.resnet1d import ResNet18, ResNet34, ResNet50, ResNet101
from ecg_encoder.model.melp.backbone import modeling_finetune
from ecg_encoder.model.melp.paths import ECGFM_PATH
try:
...
class MELPModel(MERLModel):
        ...
        if self.ecg_encoder_name == "ecgfm":
            if self.ecg_encoder_weight:
                ...
                for k, v in ckpt.items():
                    if "attn_pool_contrast" in k:
                        continue
                    new_key = k.replace("ecg_encoder.", "", 1)
                    new_ckpt[new_key] = v
    ...
    def _encode_ecg(self, ecg, normalize: bool = True):
        ...
        print(f"Rhythm (Global) shape: {proj_ecg_emb.shape}")
        print(f"Beat (Local) shape: {ecg_beat_emb.shape}")
        print(f"Token (Detail) shape: {ecg_token_emb.shape}")
    ...
    def get_text_emb(self, input_ids, attention_mask):
        if self.text_encoder_name in ["ncbi/MedCPT-Query-Encoder", "fuyingw/heart_bert"]:
            ...
            if hasattr(text_output, "last_hidden_state"):
                ...
                # CausalLMOutput 등에서도 0번 인덱스는 항상 마지막 레이어의 Hidden State입니다.
