...
class ContrastiveLoss(nn.Module):
    ...
    def forward(self, ecg_features, text_features, logit_scale, output_dict=False):
        ...
        labels = self.get_ground_truth(logits_per_ecg.shape[0], device)
...
def create_loss(args):
    return Loss(
        contrastive_weight=args.coca_contrastive_loss_weight,
        caption_weight=args.coca_caption_loss_weight,
    )